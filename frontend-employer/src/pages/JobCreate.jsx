...
const JobCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiClient.post('/employer/jobs', formData);
      addJob(response.data.data);
      navigate('/jobs');
    } catch (error) {
      console.error('Error creating job:', error);
      alert('Có lỗi xảy ra khi tạo việc làm');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Điền thông tin chi tiết để tạo tin tuyển dụng hấp dẫn
        </p>
      </div>
