...
export default function Toaster() {
  ...
  const getToastStyles = (type) => {
    switch (type) {
      case 'success':
        return {
          container: "flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow-sm dark:text-gray-400 dark:bg-gray-800",
    ...
    <div className="fixed top-4 right-4 z-50">
      {toasts.map((toast) => {
        ...
        return (
          <div
            key={toast.id}
            className={`${styles.container} animate-slide-in`}
            role="alert"
          >
            <div className={styles.icon}>
              ...
              <span className="sr-only">
                {toast.type === 'success' ? 'Check icon' : 
                 toast.type === 'error' ? 'Error icon' : 
                 toast.type === 'warning' ? 'Warning icon' : 'Info icon'}
              </span>
            </div>
